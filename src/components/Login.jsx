...
function Login() {
  ...
  async function validarUsuario() {
    ...
    const validarUsuario = usuariosRegistrados.find(
      (usuario) =>
    ...
    const validarAdmin = usuariosRegistrados.find(
      (usuario) =>
        usuario.correo === correoUsuario &&
        usuario.clave === claveUsuario &&
    ...
    if (validarUsuario) {
      ...
      if (validarAdmin) {
        navigate("/admin");
        localStorage.setItem("usuario", JSON.stringify(validarAdmin));
      } else {
        console.log("Error");
      }
    }
  ...
  return (
    <div>
      <form className="login-card">
        ...
        <button className="login-btn" type="button" onClick={validarUsuario}>
          Ingresar
        </button>

        <p className="login-foot">
          ¿No tienes cuenta? <Link to="/Formulario">Regístrate</Link>
        </p>
      </form>
    </div>
  );
